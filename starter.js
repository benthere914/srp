...
function bakePies(pieType, pieQuantity, profitMargin){
...
function returnCost(ingredients){
  return (ingredients.reduce((prev, current) => {return prev + current.cost;}, ingredients[0].cost))
}
function sellPies(pieType, pieQuantity, profitMargin) {
  let revenue = 0;
  let totalCost = 0;
  const ingredients = recipes[pieType];
  const costOfPie = returnCost(ingredients);
